...
<html lang="en">
  ...
  <body style="font-family: system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; background: #f5f5f5;">
    <div style="max-width: 520px; padding: 2rem; background: white; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); text-align: center;">
      ...
      <p style="color: #555; line-height: 1.6;">This authorization link is invalid or has expired. Please return to Garten and try again.</p>
    </div>
  </body>
</html>`;
...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (relPath === "/oauth") {
      ...
      if (error) {
        return new Response("Supabase authorization failed. Please restart the connection flow from Garten.", {
          status: 400,
...
export class GatekeeperVendor extends WorkerEntrypoint<Env> implements GatekeeperVendorIface {
  async describe(): Promise<VendorDescription> {
    return {
      ...
      description:
          "Connect your Supabase account so Garten can run SQL against your project databases, " +
