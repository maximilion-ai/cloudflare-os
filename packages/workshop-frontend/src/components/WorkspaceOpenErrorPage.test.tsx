...
describe('WorkspaceOpenErrorPage', () => {
  ...
  it('explains how to recover when access is denied without exposing workspace metadata', async () => {
    ...
    expect(renderedContainer.querySelector('h1')?.textContent).toBe("You don't have access to this space")
    expect(renderedContainer.textContent).toContain('Ask the space owner to grant you access, then try again.')
    ...
    expect(buttons.map(button => button.textContent)).toEqual(['Go to spaces', 'Try again'])
  ...
  it('gives a missing workspace a distinct, non-retryable state', async () => {
    ...
    expect(renderedContainer.querySelector('h1')?.textContent).toBe('Space not found')
    expect(renderedContainer.textContent).toContain('The link may be incorrect, or the space may have been deleted.')
    expect([...renderedContainer.querySelectorAll('button')].map(button => button.textContent))
      .toEqual(['Go to spaces'])
  })

  it('keeps unexpected failures retryable', async () => {
    const { container: renderedContainer } = await render('unexpected')

    expect(renderedContainer.querySelector('h1')?.textContent).toBe("We couldn't load this space")
    expect(renderedContainer.textContent).toContain('Try again. If the problem continues, return to your spaces.')
    expect([...renderedContainer.querySelectorAll('button')].map(button => button.textContent))
      .toEqual(['Go to spaces', 'Try again'])
  })
