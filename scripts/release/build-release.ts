...
function run(
  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    function onAbort() {
      ...
      void killProcessTreeEscalating(child.pid).catch(() => {}).then(() => {
...
async function buildAll(packages: DeployablePackage[], concurrency: number): Promise<{
  ...
  const onStopSignal = (signal: NodeJS.Signals) => {
    ...
    // Impatience, or a tree that outlived even the escalation's SIGKILL. Best-effort by now: the
    // first signal's SIGTERM may already have reparented descendants out of reach, so this is a
    // backstop and the escalation is the real guarantee.
    if (++stopSignalsReceived > 1) {
      if (forcing) return;
      forcing = true;
      void Promise.all([...running].map((child) =>
  ...
  try {
    ...
    // failed build used to leave its scratch directory behind in the tmpdir.
    rmSync(bundleDir, { recursive: true, force: true });
  }
}
